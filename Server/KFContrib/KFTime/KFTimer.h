...
namespace KFrame
{
	...
	public:
		...
		// 计时器是否已经启动
		bool IsStart() const { return _starttime != 0; }
		...
		// 下一次计时
		void NextTimer( uint64 starttime ) { SetStartTime( starttime ); }

		// 计时器停止
		void StopTimer() { _starttime = 0; }

		// 设置计时器间隔时间
		uint64 GetStartTime() const { return _starttime; }
		uint64 GetSpaceTime() const { return _keeptime; }

		void SetKeepTime( uint64 keeptime ) { _keeptime = keeptime; }
		void AddKeepTime( uint64 keeptime ) { _keeptime += keeptime; }
