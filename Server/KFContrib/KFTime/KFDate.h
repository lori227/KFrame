...
namespace KFrame
{
	...
	public:
		...
		// 天数
		uint32 GetDays() const { return static_cast< uint32 >( _time / TimeEnum::OneDaySecond ); }

		// 小时数
		uint32 GetHours() const { return GetTotalHours() - GetDays() * TimeEnum::OneDayHour; }
		uint32 GetTotalHours() const { return static_cast< uint32 >( _time / TimeEnum::OneHourSecond ); }

		// 分钟数
		uint32 GetMinutes() const { return GetTotalMinutes() - GetTotalHours() * TimeEnum::OneHourMinute; }
		uint32 GetTotalMinutes() const { return static_cast< uint32 >( _time / TimeEnum::OneMinuteSecond ); }

		// 秒数
		uint32 GetSeconds() const { return _time % TimeEnum::OneMinuteSecond; }
		uint64 GetTotalSeconds() const { return _time; }
