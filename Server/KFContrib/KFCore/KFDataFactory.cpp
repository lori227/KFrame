...
namespace KFrame
{
    ...
    KFData* KFDataFactory::CreateData( const std::string& classname, const std::string& dataname )
    {
        ...
        {
            KFLogger::LogSystem( KFLogger::Error, "[%s:%s] DataSetting Can't Find!", classname.c_str(), dataname.c_str() );
            return nullptr;
        }
    ...
    KFData* KFDataFactory::CreateData( const KFDataSetting* datasetting )
    {
        ...
        {
            KFLogger::LogSystem( KFLogger::Error, "[%s:%s] DataClass Can't Find!", datasetting->_name.c_str(), datasetting->_contain_class.c_str() );
            return nullptr;
        }
    ...
    void KFDataFactory::InitData( KFData* kfdata, const KFClassSetting* classsetting, const KFDataSetting* datasetting )
    {
        ...
        for ( auto& iter : classsetting->_static_data )
        {
            ...
            {
                KFLogger::LogSystem( KFLogger::Error, "Create Data Type Error[%s:%d]!", childdatasetting->_name.c_str(), childdatasetting->_type );
                continue;
            }
