...
namespace KFrame
{
	...
	__KF_MESSAGE_FUNCTION__( KFCompoundModule::HandleCompoundDataReq )
	{
		...
		// 判断是否足够
		if ( player->CheckAgentData( &kfsetting->_cost_data, __FUNCTION_LINE__ ) )
		{
			return _kf_display->SendToClient( player, KFMsg::DataNotEnough );
		}

		// 扣除材料
		player->RemoveAgentData( &kfsetting->_cost_data, __FUNCTION_LINE__ );

		// 添加合成的属性
		player->AddAgentData( &kfsetting->_compound_data, 1.0f, true, __FUNCTION_LINE__ );
	}
}