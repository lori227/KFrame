...
#define __SEND_MESSAGE_TO_CLIENT__( msgid, message ) _kf_cluster_shard->SendMessageToClient( kfguid, msgid, message )
	...
	{
		_kf_config->AddConfig( _kf_data_config, _kf_plugin->_plugin_name, _kf_plugin->_config, true );
	}

	void KFDataShardModule::BeforeRun()
	{
		_kf_tcp_server->RegisterDiscoverFunction( this, &KFDataShardModule::OnServerDiscoverClient );
