...
deploypath=/data/upload/deploy/battle/linux
...
copyfile 1 battle.master.startup
copyfile 1 battle.proxy.startup
copyfile 1 battle.shard.startup
copyfile 1 common.startup

#config
path1=$versionpath/config
path2=$deploypath/config
copyfile 1 battle.config

#plugin
path1=$versionpath/bin
path2=$deploypath
copyfile 1 KFStartup battleserver
copyfile 1 KFStartupd battleserverd
...
copyfile 1 KFBattleProxy.so
copyfile 1 KFBattleProxyd.so
copyfile 1 KFBattleShard.so
...
#upload to ftp
curl -sd '{"ftpid":'$2',"apppath":"battle"}' $3
