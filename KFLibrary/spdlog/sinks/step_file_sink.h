...
namespace spdlog {
    ...
    #define MKDIR(a) mkdir((a),0755)
        ...
        struct default_step_file_name_calculator
        {
            ...
            static std::tuple<filename_t, filename_t> calc_filename( const filename_t& filename, unsigned filecount )
            {
                ...
                filename_t newfile = fmt::format( "{}_{:04d}-{:02d}-{:02d}_{:02d}_{}{}", basename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, filecount, ext );
        ...
        class step_file_sink final : public base_sink<Mutex>
        {
        public:
            step_file_sink( filename_t base_filename, unsigned step_seconds, unsigned max_size )
                ...
                , _file_count( 1 )
            {
                ...
                _tp = _next_tp();
                std::tie( _current_filename, _ext ) = FileNameCalc::calc_filename( _base_filename, _file_count );
        ...
        protected:
            void sink_it_( const details::log_msg& msg ) override
            {
                ...
                if ( std::chrono::system_clock::now() >= _tp || _current_size + msg_size > _max_size )
                {
                    ...
                    std::tie( new_filename, std::ignore ) = FileNameCalc::calc_filename( _base_filename, _file_count );
