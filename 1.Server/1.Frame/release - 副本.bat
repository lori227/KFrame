...
set pluginpath=KFPlugin
if not exist %releaseepath%\%pluginpath% (mkdir %releaseepath%\%pluginpath% )
cd %localpath%\%pluginpath%
for /d %%i in (* ) do (
	echo %%i
	if not exist ..\..\%releaseepath%\%pluginpath%\%%i (mkdir ..\..\%releaseepath%\%pluginpath%\%%i )
	copy /y %%i\*Interface.h.h ..\..\%releaseepath%\%pluginpath%\%%i	
)
cd ..\..\
