...
namespace KFrame
{
    ...
    __KF_ADD_ELEMENT_FUNCTION__( KFHeroModule::AddHeroElement )
    {
        ...
        // 判断是否存在该英雄
        auto kfhero = kfdata->FindData( kfelementobject->_config_id );
        if ( kfhero == nullptr )
        {
            // 不存在, 创建
            kfhero = _kf_kernel->CreateObject( kfdata->GetDataSetting() );

            // 添加英雄
            player->AddData( kfdata, kfelementobject->_config_id, kfhero );
        }
        else
        {
            // 存在, 判断有效时间
            ...
            if ( datatime != _invalid_int )
            {
                ...
                {
                    // 时限英雄
                    player->UpdateData( kfhero, __KF_STRING__( time ), KFOperateEnum::Add, elementtime );
                }
            }
            else
            {
