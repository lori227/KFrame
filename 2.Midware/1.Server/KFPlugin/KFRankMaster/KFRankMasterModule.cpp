...
namespace KFrame
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    __KF_SERVER_DISCOVER_FUNCTION__( KFRankMasterModule::OnServerDisconverShard )
    {
        if ( handlename == __KF_STRING__( rank ) && handletype == __KF_STRING__( shard ) )
        {
            _rank_shard_hash.AddHashNode( handlename, handleid, 100 );
        }
    }

    __KF_SERVER_LOST_FUNCTION__( KFRankMasterModule::OnServerLostShard )
    {
        if ( handlename == __KF_STRING__( rank ) && handletype == __KF_STRING__( shard ) )
        {
            _rank_shard_hash.RemoveHashNode( handleid );
        }
    }
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void KFRankMasterModule::StartRefreshRankDataTimer( const KFRankSetting* kfsetting )
    {
        if ( kfsetting->_refresh_type == KFTimeEnum::None )
        {
            ...
            __REGISTER_LOOP_TIMER__( kfsetting->_rank_id, kfsetting->_refresh_time * 1000, &KFRankMasterModule::OnTimerRefreshRankData );
        }
        else
        {
            ...
            __REGISTER_SCHEDULE_FUNCTION__( kfschedulesetting, &KFRankMasterModule::OnScheduleRefreshRankData );
        }
    }
