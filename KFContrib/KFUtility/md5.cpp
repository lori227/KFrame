...
#include <openssl/md5.h>
    ...
    void md5bin( const void* data, uint32 length, unsigned char out[ 16 ] )
    {
        MD5_CTX c;
        MD5_Init( &c );
    ...
    std::string Md5::Md5File( const std::string& filename )
    {
        ...
        auto res = md5file( file );
        std::fclose( file );
        return res;
    }


    std::string Md5::Md5Data( const void* data, uint32 length )
    {
        std::string res;
        unsigned char out[ 16 ];
        md5bin( data, length, out );
        for ( size_t i = 0; i < 16; ++i ) {
            res.push_back( hb2hex( out[ i ] >> 4 ) );
            res.push_back( hb2hex( out[ i ] ) );
        }
    ...
    std::string Md5::Md5Data( const std::string& data )
    {
        return Md5Data( data.c_str(), ( uint32 )data.length() );
    }

    /* Generate shorter md5sum by something like base62 instead of base16 or base10. 0~61 are represented by 0-9a-zA-Z */
    std::string Md5::md5Sum6( const void* data, uint32 length )
    {
    ...
    std::string Md5::md5Sum6( const std::string& data )
    {
        return md5Sum6( data.c_str(), ( uint32 )data.length() );
    }
}