...
namespace KFrame
{
    ...
    void KFHttpCommon::MakeSignature( KFJson& json )
    {
        ...
        json.AddMember( rapidjson::StringRef( __STRING__( sign ).c_str() ), md5sign, json.GetAllocator() );
    }

    bool KFHttpCommon::VerifySignature( KFJson& json )
    {
        auto sign = __JSON_GET_STRING__( json, __STRING__( sign ) );
        __JSON_REMOVE__( json, __STRING__( sign ) );
    ...
    std::string KFHttpCommon::SendResponseCode( uint32 code )
    {
        ...
        __JSON_SET_VALUE__( response, __STRING__( retcode ), code );
        return SendResponse( response );
    }

    uint32 KFHttpCommon::GetResponseCode( KFJson& json )
    {
        if ( !__JSON_HAS_MEMBER__( json, __STRING__( retcode ) ) )
        {
            return _invalid_int;
        }

        return __JSON_GET_UINT32__( json, __STRING__( retcode ) );
    }
    ...
    std::string KFHttpCommon::SendResponse( KFJson& json )
    {
        if ( !__JSON_HAS_MEMBER__( json, __STRING__( retcode ) ) )
        {
            json.AddMember( rapidjson::StringRef( __STRING__( retcode ).c_str() ), KFEnum::Ok, json.GetAllocator() );
        }
    ...
    {
        json.AddMember( rapidjson::StringRef( __STRING__( retcode ).c_str() ), code, json.GetAllocator() );
        return __JSON_SERIALIZE__( json );
    }
}
