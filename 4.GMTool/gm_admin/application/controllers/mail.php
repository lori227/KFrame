...
class Mail extends MY_Controller {
    ...
    function index($page=1){
       $queryParams = array(
        ...
        //$data['emailFlag'] = $this->config->item('emailFlag', 'adconfig');
    ...
    public function add(){
        //获取用户列表
        $emailJson = $this->redis->get(self::USER_MAIL_LIST);
        $data['redisEmail'] =array();
        if($emailJson){
            $data['redisEmail'] = array_values(json_decode($emailJson,true));
        }
        if($this->input->is_post()){
            $type = intval($this->input->post('type'));//邮件类型，1.gm邮件；2.用户补偿邮件
            $title = trim($this->input->post('title'));
            $content = trim($this->input->post('content'));
            $days = intval(trim($this->input->post('expire')));//邮件过期时间点
            ...
            $this->server->createEmail($insert);
            //插入mysql
            $insert['status'] = 0;//未审核
            $insert['add_time'] = $now;
            $insert['add_user'] = $this->userData['uid'];
            if($type > 1){
                $insert['playerIdStr'] = json_encode($data['redisEmail']);//用户邮件的用户ID列表
            }
            $this->db->insert('mail', $insert); //插入mysql
    ...
    public function modify($eid){
        ...
        $data['emailType'] = $this->config->item('emailType', 'adconfig');
        $data['redisEmail'] = json_decode($data['playerIdStr'],true);//获取用户列表
