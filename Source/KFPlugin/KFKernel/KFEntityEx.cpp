...
namespace KFrame
{
	...
	void KFEntityEx::SetKeyID( uint64 id )
	{
		...
		_self_id_string = KFUtility::ToString< uint64 >( id );
	}
	...
	const char* KFEntityEx::GetKeyString()
	{
		return _self_id_string.c_str();
	}
	...
	{
		KFLogger::LogLogic( KFLogger::Info, "[ %s:%u ] player[ %s ] add agent[%0.2f][ %s ]",
			function, line, _self_id_string.c_str(), multiple, kfagents->_string.c_str() );
	...
	void KFEntityEx::AddAgentData( const KFAgent* kfagent, float multiple, const char* function, uint32 line )
	{
		...
		{
			return KFLogger::LogSystem( KFLogger::Error, "[ %s:%u ] can't find data setting [ %s ]! ", 
		...
		else
		{
			...
			case KFDataDefine::Type_Vector3D:
				KFLogger::LogLogic( KFLogger::Info, "[ %s:%u ] error data type[ %s ]! ", 
					function, line, kfagent->_string.c_str() );
				break;
	...
	void KFEntityEx::AddObjectAgentData( KFAgent* kfagent, float multiple, const char* function, uint32 line )
	{
		...
		if ( kfparent == nullptr )
		{
			return KFLogger::LogSystem( KFLogger::Error, "[%s:%u] can't find data [ %s ]!", 
				function, line, kfagent->_string.c_str() );
		}
	...
	void KFEntityEx::AddRecordAgentData( KFAgent* kfagent, float multiple, const char* function, uint32 line )
	{
		...
		{
			return KFLogger::LogSystem( KFLogger::Error, "[%s:%u] can't find config id [ %s ]!", 
				function, line, kfagent->_string.c_str() );
		}
		...
		{
			return KFLogger::LogSystem( KFLogger::Error, "[%s:%u] can't find data [ %s ]!", 
				kfagent->_string.c_str() );
		}

		auto kfdata = kfparent->FindData( configid );
		if ( kfdata == nullptr )
		{
			UpdateData( kfparent, configid, KFField::_id, KFOperateEnum::Set, configid );
		}
	...
	bool KFEntityEx::CheckAgentData( const KFAgent* kfagent, const char* function, uint32 line )
	{
		...
		if ( datasetting == nullptr )
		{
			KFLogger::LogSystem( KFLogger::Error, "[%s:%u] can't find data setting [ %s ]! ", 
				function, line, kfagent->_string.c_str() );
			return false;
		}
		...
		case KFDataDefine::Type_String:
		case KFDataDefine::Type_Vector3D:
			KFLogger::LogSystem( KFLogger::Error, "[%s:%u] data type error [ %s ]! ",
				function, line, kfagent->_string.c_str() );
			break;
		case KFDataDefine::Type_Record:
	...
	void KFEntityEx::RemoveAgentData( const KFAgent* kfagent, const char* function, uint32 line )
	{
		...
		{
			KFLogger::LogLogic( KFLogger::Info, "[%s:%u] [ %s ] remove data [ %s ]",
				function, line, _self_id_string.c_str(), kfagent->_string.c_str() );
		}
	}
	...
	KFMsg::PBObject* KFEntityEx::FindSyncUpdateFromPBObject( KFData* kfdata, KFMsg::PBObject* pbobject )
	{
		auto datasetting = kfdata->GetDataSetting();
	...
	KFMsg::PBObject* KFEntityEx::FindSyncUpdateFromPBRecord( KFData* kfdata, KFMsg::PBObject* pbobject )
	{
		// 查找记录
		auto pbrecord = FindRecordFromPBObject( kfdata, pbobject );
	...
	void KFEntityEx::SyncAddData( KFData* kfdata, uint64 key )
	{
		...
		// 不需要同步
		auto datasetting = kfdata->GetDataSetting();
		if ( !datasetting->HaveFlagMask( KFDataDefine::Mask_Sync_Client ) )
		{
			return;
		}
		...
		do
		{
			...
			{
				pbobject = FindSyncUpdateFromPBRecord( savedata, pbobject );
	...
	void KFEntityEx::SyncRemoveData( KFData* kfdata, uint64 key )
	{
		...
		do
		{
			...
			if ( savedata->GetType() == KFDataDefine::Type_Object )
			{
				FindSyncUpdateFromPBRecord( savedata, pbobject );
			}
		} while ( !datahierarchy.empty() );
	...
	void KFEntityEx::SyncUpdateData( KFData* kfdata, uint64 key )
	{
		...
		do
		{
			...
			auto datasetting = savedata->GetDataSetting();
			switch ( datasetting->_type )
			{
				case KFDataDefine::Type_Object:
				{
					pbobject = FindSyncUpdateFromPBObject( savedata, pbobject );
					break;
				}
				case KFDataDefine::Type_Record:
				{
					if ( datahierarchy.empty() )
					{
						KFLogger::LogSystem( KFLogger::Error, "[%s] data[%s] datahierarchy empty!", __FUNCTION__, datasetting->_name.c_str() );
						return;
					}
