...
namespace KFrame
{
    ...
    public:
        ...
        // 注册world到目录服务器
        virtual bool WorldRegisterToDir( uint64 world_id, const std::string& url );

        // 更新gate的在线数量
        virtual void UpdateGateToDir( uint64 app_id, const std::string& ip, uint32 port, uint32 count, uint32 expire_time );
