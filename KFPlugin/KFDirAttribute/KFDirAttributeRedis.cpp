...
#define __DIR_REDIS_DRIVER__ _kf_redis->Create( __STRING__( dir ) )
    ...
    bool KFDirAttributeRedis::ZoneRegister( uint32 zoneid, KFJson& zonedata )
    {
        ...
        redisdriver->HMSet( __REDIS_KEY_2__(  __STRING__( zone ), zoneid ), values );
    ...
    bool KFDirAttributeRedis::ZoneUpdate( uint64 appid, uint32 zoneid, uint32 count, const std::string& ip, uint32 port, uint32 expiretime )
    {
        // 判断最小负载
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto strkey = __REDIS_KEY_2__( __STRING__( gate ), zoneid );
    ...
    {
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto kfresult = redisdriver->HGetUInt64( __REDIS_KEY_2__( __STRING__( zone ), zoneid ), __STRING__( status ) );
    ...
    bool KFDirAttributeRedis::UpdateZoneStatus( uint32 zoneid, uint32 status )
    {
        ...
        if ( zoneid != 0u )
        {
            auto kfresult = redisdriver->HSet( __REDIS_KEY_2__( __STRING__( zone ), zoneid ), __STRING__( status ), status );
            return kfresult->IsOk();
        }
        ...
        {
            redisdriver->HSet( __REDIS_KEY_2__( __STRING__( zone ), zonepair.first ), __STRING__( status ), status );
        }
        auto kfresult = redisdriver->WriteExec();
        return kfresult->IsOk();
    }
    ...
    StringMap KFDirAttributeRedis::QueryZoneIp( uint32 zoneid )
    {
        ...
        auto kfqueryloginid = redisdriver->HGetUInt64( __REDIS_KEY_2__( __STRING__( zone ), zoneid ), __STRING__( loginzoneid ) );
        if ( kfqueryloginid->_value == 0u )
        {
            return zonedata;
        }

        auto kfresult = redisdriver->HGetAll( __REDIS_KEY_2__( __STRING__( gate ), kfqueryloginid->_value ) );
    ...
    StringMap KFDirAttributeRedis::QueryZoneData( uint32 zoneid )
    {
        StringMap zonedata;
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto kfzonedata = redisdriver->HGetAll( __REDIS_KEY_2__( __STRING__( zone ), zoneid ) );
        ...
        auto kfgatedata = redisdriver->HGetAll( __REDIS_KEY_2__( __STRING__( gate ), loginzoneid ) );
    ...
    bool KFDirAttributeRedis::SetZoneRecommend( const std::string& flag, uint32 zoneid, bool isrecommend )
    {
        ...
        if ( isrecommend )
        {
            auto kfresult = redisdriver->SAdd(  __REDIS_KEY_2__( __STRING__( zonerecommendlist ), flag ), zoneid );
            ok = kfresult->IsOk();
        }
        else
        {
            auto kfresult = redisdriver->SRem( __REDIS_KEY_2__( __STRING__( zonerecommendlist ), flag ), zoneid );
            ok = kfresult->IsOk();
        }
    ...
    {
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto kfresult = redisdriver->SRandMember( __REDIS_KEY_2__( __STRING__( zonerecommendlist ), flag ) );
    ...
    uint32 KFDirAttributeRedis::BalanceAllocZone( const std::string& flag )
    {
        ...
        {
            auto kfloginzoneid = redisdriver->HGetUInt64( __REDIS_KEY_2__( __STRING__( zone ), zonepair.first ), __STRING__( loginzoneid ) );
            if ( kfloginzoneid->_value == 0u )
            {
                continue;
            }

            auto kfqueryip = redisdriver->HGet( __REDIS_KEY_2__( __STRING__( gate ), kfloginzoneid->_value ), __STRING__( ip ) );
    ...
    {
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto kfresult = redisdriver->Set( __REDIS_KEY_2__( __STRING__( worldurl ), worldid ), url );
        return kfresult->IsOk();
    }
    ...
    {
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto kfresult = redisdriver->Get( __REDIS_KEY_2__( __STRING__( worldurl ), worldid ) );
        return kfresult->_value;
    }
    ...
    {
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto strmasterkey = __REDIS_KEY_2__( __STRING__( master ), appid );
        ...
        redisdriver->SAdd( __REDIS_KEY_3__( __STRING__( master ), appname, zoneid ), appid );
        auto kfresult = redisdriver->WriteExec();
        return kfresult->IsOk();
    }
    ...
    StringMap KFDirAttributeRedis::QueryMasterIp( const std::string& appname, uint32 zoneid )
    {
        StringMap zonedata;
        auto redisdriver = __DIR_REDIS_DRIVER__;
        auto strmasternamekey = __REDIS_KEY_3__( __STRING__( master ), appname, zoneid );

        auto kfappid = redisdriver->SRandMember( strmasternamekey );
        if ( !kfappid->_value.empty() )
        {
            auto kfmasterdata = redisdriver->HGetAll( __REDIS_KEY_2__( __STRING__( master ), kfappid->_value ) );
    ...
    StringMapList KFDirAttributeRedis::QueryMasterList( const std::string& appname, uint32 zoneid )
    {
        ...
        {
            auto kfmasterdata = redisdriver->HGetAll( __REDIS_KEY_2__( __STRING__( master ), appid ) );
            if ( !kfmasterdata->_value.empty() )
            {
