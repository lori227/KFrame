...
namespace KFrame
{
    ...
    protected:
        ...
        // 设置任务时间
        void SetTaskTime( KFEntity* player, DataPtr kftask, const KFTaskSetting* kfsetting, uint64 validtime, bool update );
        // 启动任务超时定时器
        void StartTaskTimeoutTimer( KFEntity* player, uint32 taskid, uint64 time );
        void StopTaskTimeoutTimer( KFEntity* player, DataPtr kftask, uint32 taskid );

        // 开启任务
        DataPtr CreateTask( KFEntity* player, const KFTaskSetting* kfsetting, uint32 status, uint64 validtime = 0u );
        void UpdataTaskStatus( KFEntity* player, const KFTaskSetting* kfsetting, DataPtr kftask, uint32 status, uint64 time );

        // 任务条件完成
        void DoneTask( KFEntity* player, DataPtr kftask, const KFTaskSetting* kfsetting, bool update );

        // 添加到完成任务列表
        void AddFinishTask( KFEntity* player, DataPtr kftask, const KFTaskSetting* kfsetting, bool update );
        __KF_TIMER_FUNCTION__( OnTimerTaskFinish );

        // 任务交付完成
        void FinishTask( KFEntity* player, uint32 taskid );
        void FinishTask( KFEntity* player, DataPtr kftask, const KFTaskSetting* kfsetting );
        ...
        // 初始化任务条件
        void InitTaskCondition( KFEntity* player, const KFTaskSetting* kfsetting, DataPtr kftask, uint32 status, bool update );
