...
namespace KFrame
{
    ...
    public:
        // 注册重置逻辑
        template< class T >
        void RegisterResetFunction( const KFTimeData& timedata, T* object, void ( T::*handle )( KFEntity*, uint64, uint64 ) )
        {
            KFResetFunction function = std::bind( handle, object, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3 );
            AddResetFunction( timedata, typeid( T ).name(), function );
        }
    ...
    protected:
        // 添加重置函数
        virtual void AddResetFunction( const KFTimeData& timedata, const std::string& module, KFResetFunction& function ) = 0;
...
#define __REGISTER_RESET__( timedata, function ) _kf_reset->RegisterResetFunction( timedata, this, function )
