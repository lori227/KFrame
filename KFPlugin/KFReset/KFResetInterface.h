...
namespace KFrame
{
    ...
    public:
        // 注册重置逻辑
        template<class T>
        void RegisterResetFunction( const std::string& functionname, uint32 count, T* object, void ( T::*handle )( EntityPtr, uint32, uint64, uint64 ) )
        {
        ...
        // 获得下一次重置时间
        virtual uint64 CalcNextResetTime( EntityPtr player, uint32 timeid ) = 0;

        // 设置重置时间
        virtual void SetResetTime( EntityPtr player, uint32 timeid, uint64 now_time ) = 0;
    protected:
        // 添加重置函数
...
#define __KF_RESET_FUNCTION__( function ) void function( EntityPtr player, uint32 timeid, uint64 lastresettime, uint64 nowresettime )
