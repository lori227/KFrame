...
namespace KFrame
{
    ...
    public:
        // 注册重置逻辑
        template< class T >
        void RegisterResetFunction( const KFTimeData& timedata, uint32 count, T* object, void ( T::*handle )( KFEntity*, const KFTimeData&, uint64, uint64 ) )
        {
            ...
            AddResetFunction( timedata, count, typeid( T ).name(), function );
        }
    ...
    protected:
        // 添加重置函数
        virtual void AddResetFunction( const KFTimeData& timedata, uint32 count, const std::string& module, KFResetFunction& function ) = 0;
...
#define __REGISTER_RESET__( timedata, function ) _kf_reset->RegisterResetFunction( timedata, 1u, this, function )
#define __REGISTER_RESET_COUNT__( timedata, count, function ) _kf_reset->RegisterResetFunction( timedata, count, this, function )
