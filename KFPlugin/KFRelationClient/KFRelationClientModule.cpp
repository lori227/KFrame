...
#define __RELATION_ROUTE_NAME__ __STRING__( relation )
    ...
    void KFRelationClientModule::PBRelationToKFData( const KFMsg::PBRelation* pbrelation, DataPtr kfrelation )
    {
        ...
        kfrelation->SetKeyID( pbrelation->player_id() );
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    __KF_MESSAGE_FUNCTION__( KFRelationClientModule::HandleQueryRelationToGameAck, KFMsg::S2SQueryRelationToGameAck )
    {
        ...
        auto kfrecord = kfentity->Find( setting->_id );
        if ( kfrecord == nullptr )
        {
        ...
        {
            // 通知关系列表,我上线了
            EnterLeaveUpdateToRelation( kfentity, setting->_id );
        }
    }
    ...
    {
        auto kfrelation = kfentity->Find( kfmsg->relationname(), kfmsg->player_id() );
    ...
    {
        // 不能添加自己
        if ( kfentity->GetKeyID() == kfmsg->player_id() || kfmsg->player_id() == 0u )
        {
            return;
        }
        ...
        auto kfrelationrecord = kfentity->Find( setting->_id );
        ...
        // 已经是关系, 不能申请
        auto kfrelation = kfrelationrecord->Find( kfmsg->player_id() );
        if ( kfrelation != nullptr )
        {
        ...
        {
            return _kf_display->SendToClient( kfentity, KFMsg::RelationSelfLimit, setting->_max_count );
        }

        // 不需要申请, 直接添加
        if ( setting->_invite_data_name.empty() )
        {
            return AddRelationToRelation( kfentity, kfmsg->player_id(), kfmsg->playername(), kfrelationrecord, setting );
        }
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // 检查屏蔽字
        ...
        req.set_playerid( kfmsg->player_id() );
    ...
    __KF_MESSAGE_FUNCTION__( KFRelationClientModule::HandleReplyRelationInviteReq, KFMsg::MsgReplyRelationInviteReq )
    {
        ...
        auto kfinviterecord = kfentity->Find( setting->_invite_data_name );
        if ( kfinviterecord == nullptr )
        {
        ...
        // player_id 为0, 代表全部操作
        if ( kfmsg->player_id() == _invalid_int )
        {
            ReplyRelationAllInvite( kfentity, kfinviterecord, setting, kfmsg->operate() );
        }
        else
        {
            ReplyRelationInvite( kfentity, kfinviterecord, setting, kfmsg->operate(), kfmsg->player_id() );
        }
    }
    ...
    __KF_MESSAGE_FUNCTION__( KFRelationClientModule::HandleAddRelationToGameAck, KFMsg::S2SAddRelationToGameAck )
    {
        ...
        if ( !setting->_invite_data_name.empty() )
        {
            kfentity->RemoveRecord( setting->_invite_data_name, kfrelation->GetKeyID() );
        }
    ...
    //        req.set_playerid( pbscore->player_id() );
    //        req.mutable_members()->CopyFrom( pbscore->members() );
    //
    //        auto pbstrings = req.mutable_pbdata();
    //        __ADD_PB_STRING__( __STRING__( id ), __TO_STRING__( pbscore->player_id() ) );
